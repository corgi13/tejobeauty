  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  onModuleInit() {
    // Start monitoring when the module initializes
    this.startMonitoring();
  }

  onModuleDestroy() {
    // Stop monitoring when the module is destroyed
    this.stopMonitoring();
  }
   ...
   * In a production environment, this would collect actual system metrics
   * For development/testing, we'll use a mix of real and mock data
   */
  private async collectMetrics(): Promise<void> {
    try {
      ...
      // For metrics we can't easily get in all environments, use mock data
      const errorMetrics = this.generateMockErrorMetrics();

      // Try to get some real database metrics
      ...
      // Fallback to mock data if real metrics collection fails
  ...
  /**
   * Collect real system metrics where possible
   */
  private async collectRealSystemMetrics(): Promise<SystemMetrics> {
    // Get CPU usage
    ...
    // Get disk usage (simplified, in production would check specific mount points)
    // For now, use mock data for disk
    ...
    return {
      cpu: {
        ...
        temperature: undefined, // Would require additional libraries or system-specific commands
      },
      memory: {
        total: totalMem,
        used: usedMem,
  ...
  private async getCpuUsage(): Promise<number> {
    return new Promise((resolve) => {
      ...
      if (this.previousCpuInfo) {
        ...
        const usage =
          totalDiff > 0 ? 100 - (idleDiff / totalDiff) * 100 : 0;
        ...
        this.previousCpuInfo = { idle, total };
        // First run, return a reasonable default
        resolve(0); // Return 0 on first call, will be accurate on the next one.
      }
    });
  }
  ...
  private async getNetworkStats(): Promise<{
    ...
    // In a real implementation, would use system-specific methods to get actual network stats
  ...
  private async collectPerformanceMetrics(): Promise<PerformanceMetrics> {
    try {
      // In a real implementation, would collect actual database metrics
      // For now, use a mix of real and mock data
      ...
      return {
        ...
        databasePerformance: {
          ...
          connectionPoolSize: 20, // Would get from Prisma config in production
          activeConnections,
  ...
  private async getActiveConnections(): Promise<number> {
    try {
      // This query requires sufficient database privileges.
      // It returns an array with an object like [{ count: '5' }]
      // const result: { count: bigint }[] =
      //   await this.prisma.$queryRaw`SELECT count(*) FROM pg_stat_activity WHERE datname = current_database() AND state = 'active';`;
      //
      // if (result.length > 0 && result[0].count) {
      //   return Number(result[0].count);
      // }
      // return 0;
      return Math.floor(Math.random() * 15) + 5; // Using mock data for now: 5-20 connections
    } catch (error) {
    ...
    try {
      ...
      const results = await Promise.all(
        services.map(async (name) => {
          try {
            // For database, try to perform an actual check
            if (name === "database") {
            ...
            // For other services, simulate checks
            // In production, would make actual HTTP requests to service health endpoints
  ...
  private checkAlertConditions(
    ...
    const downServices = this.serviceStatuses.filter(
      (s) => s.status === "down",
    );
    for (const service of downServices) {
      this.addAlert(
        "critical",
        `Service down: ${service.name} - ${service.message}`,
      );
    }
    ...
    for (const service of degradedServices) {
      this.addAlert(
        "warning",
        `Service degraded: ${service.name} - ${service.message}`,
      );
    }
  }
  ...
  private initializeMockData(): void {
    ...
    if (Math.random() < 0.2) {
      this.addAlert(
        "critical",
        "Service down: email-service - Service is not responding",
      );
    }
  }
